...
Modal split and Pkm are matched by person ID and one-based standard-output trip number to every final selected-plan main trip. They retain MATSim `output_trips` `traveled_distance`: complete routed main-trip distance, including PT access, egress and transfers. Missing distance is never replaced by a beeline. The files report sample counts and Pkm, factor-20 demand expansions, shares, means, medians, travel times, coverage and exclusions separately for both scopes. Bike-km is explicitly derived as bike Pkm under a one-person-per-bike convention. Walk remains Pkm and is never described as vehicle-kilometres.
